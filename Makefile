APP_NAME := dachcourier-admin-backend
BIN_DIR := bin
BIN := $(BIN_DIR)/$(APP_NAME)
GOCACHE ?= $(CURDIR)/.cache/go-build
MIGRATE_ARGS := $(filter-out migrate up,$(MAKECMDGOALS))
COMPOSE := docker compose --project-name dachcourier -f devops/docker-compose.yml
DEV_COMPOSE := docker compose --project-name dachcourier-dev -f devops/docker-compose.dev.yml
# PRODUCTION_COMPOSE
...
dev-db-up:
	$(COMPOSE) up -d dachcourier-postgres

dev-up:
	$(DEV_COMPOSE) up -d --build

dev-migrate:
	$(DEV_COMPOSE) run --rm dachcourier-admin-backend dachcourier-admin-migrate up

dev-down:
	$(DEV_COMPOSE) down

dev-logs:
	$(DEV_COMPOSE) logs -f dachcourier-admin-backend
...
prod-logs-api:
	$(PROD_COMPOSE) logs -f dachcourier-admin-backend

prod-logs-db:
	$(PROD_COMPOSE) logs -f dachcourier-postgres
...
docker-logs:
	$(COMPOSE) logs -f dachcourier-admin-backend
